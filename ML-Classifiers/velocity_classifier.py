...
This system trains and classifies head velocity patterns for wheelchair navigation
using 7 speed categories. Implements Proximity Forest 2.0 with enhanced
...
Author: Manuel Alejandro Dlegadnro elgado Ramos 
...
class MSM_Enhanced:
    ...
    def compute_distance(self, seq1: Tuple[np.ndarray, np.ndarray], 
        ...
        seq1, seq2 : tuple of np.ndarray
            Bivariate sequences (velocity_x, velocity_y)
...
class VelocityParameters:
    """Parameters for synthetic velocity generation"""
    mean: float
    std: float


class SyntheticVelocityGenerator:
    """
    Generates synthetic head velocity sequences for speed classification
    
    Produces training data for 7 speed categories based on head velocity
    magnitudes during wheelchair navigation.
    """
    
    VELOCITY_CONFIGS = {
        'MUY_BAJA': VelocityParameters(mean=2.0, std=0.8),
        'BAJA': VelocityParameters(mean=12.0, std=2.5),
        'MEDIA_BAJA': VelocityParameters(mean=24.0, std=3.0),
        'MEDIA': VelocityParameters(mean=37.0, std=3.5),
        'MEDIA_ALTA': VelocityParameters(mean=48.0, std=4.0),
        'ALTA': VelocityParameters(mean=60.0, std=4.5),
        'MUY_ALTA': VelocityParameters(mean=84.0, std=6.0)
    }
    ...
    def generate_category_dataset(self, category: str, n_samples: int) -> List[Tuple[np.ndarray, np.ndarray]]:
        """
        Generate synthetic dataset for a specific speed category
        
        Parameters
        ----------
        category : str
            Category name (must be in VELOCITY_CONFIGS)
        n_samples : int
        ...
        -------
        list of tuple
            List of (HeadVelocityX, HeadVelocityY) sequences
        """
        if category not in self.VELOCITY_CONFIGS:
            raise ValueError(f"Unknown category: {category}")
        
        config = self.VELOCITY_CONFIGS[category]
        sequences = []
        
        for _ in range(n_samples):
            # Generate base velocities with random direction
            angle = np.random.uniform(0, 2 * np.pi)
            magnitude_base = np.random.normal(config.mean, config.std, self.sequence_length)
            
            # Ensure non-negative magnitudes
            magnitude_base = np.abs(magnitude_base)
            
            # Add temporal variation
            t = np.linspace(0, 1, self.sequence_length)
            magnitude_variation = 1.0 + 0.2 * np.sin(2 * np.pi * t)
            magnitude = magnitude_base * magnitude_variation
            ...
            # Add small noise
            vel_x += np.random.normal(0, config.std * 0.1, self.sequence_length)
            vel_y += np.random.normal(0, config.std * 0.1, self.sequence_length)
            
            sequences.append((vel_x, vel_y))
        
        return sequences
    
    def generate_full_dataset(self, n_samples_per_category: int) -> Dict[str, List[Tuple[np.ndarray, np.ndarray]]]:
        """
        Generate complete dataset for all 7 categories
        ...
        for category in self.VELOCITY_CONFIGS.keys():
...
class ProximityForest:
    ...
    def __init__(self, n_trees: int = 100, max_depth: int = 15,
    ...
    def fit(self, X: List[Tuple[np.ndarray, np.ndarray]], y: np.ndarray):
        ...
        ----------
        X : list of tuple
            Training sequences (VelocityX, VelocityY)
        y : np.ndarray
            Training labels
        """
...
class TrainingPipeline:
    ...
    def __init__(self, output_dir: str = "trained_velocity_models"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        self.generator = SyntheticVelocityGenerator()
        self.model = None
        self.command_mapping = None
        self.training_metadata = None
    
    def run(self, n_samples_per_category: int = 24, n_trees: int = 100, 
            max_depth: int = 15, test_split: float = 0.2):
        """
        ...
        """
        print("\n" + "="*80)
        print("HEAD VELOCITY SPEED CLASSIFIER - TRAINING PIPELINE")
        print("="*80 + "\n")
        
        # Generate data
        print("Generating synthetic velocity data...")
    ...
    def _collect_metadata(self, dataset: Dict, accuracy: float, training_time: float,
        ...
        for cat_name, sequences in dataset.items():
            ...
            magnitudes = np.sqrt(x_values**2 + y_values**2)
            
            command_stats[cat_name] = {
                'count': len(sequences),
                'velocity_x_mean': float(np.mean(x_values)),
                'velocity_y_mean': float(np.mean(y_values)),
    ...
    def _generate_pkl_files(self, dataset: Dict):
        ...
        # 1. pf2_velocidad.pkl (references in original format)
        velocity_references = {}
        for cat_name, sequences in dataset.items():
            # Store as list of magnitude arrays (to match original format)
            refs = []
            for seq in sequences[:5]:  # Store 5 reference sequences
                # Combine X and Y into magnitude
                magnitude = np.sqrt(seq[0]**2 + seq[1]**2)
                refs.append(magnitude)
            velocity_references[cat_name] = refs
        
        with open(self.output_dir / 'pf2_velocidad.pkl', 'wb') as f:
            pickle.dump(velocity_references, f)
        print(f"  Generated: pf2_velocidad.pkl")
        
        # 2. command_mapping_velocity.pkl
        with open(self.output_dir / 'command_mapping_velocity.pkl', 'wb') as f:
            pickle.dump(self.command_mapping, f)
        print(f"  Generated: command_mapping_velocity.pkl")
        
        # 3. velocity_schema.pkl
        schema = {
            'features': ['HeadVelocityX', 'HeadVelocityY'],
            'sequence_length': self.generator.sequence_length,
            'expected_dtypes': ['float64', 'float64']
        }
        
        with open(self.output_dir / 'velocity_schema.pkl', 'wb') as f:
            pickle.dump(schema, f)
        print(f"  Generated: velocity_schema.pkl")
        
        # 4. training_metadata_velocity.pkl
        with open(self.output_dir / 'training_metadata_velocity.pkl', 'wb') as f:
            pickle.dump(self.training_metadata, f)
        print(f"  Generated: training_metadata_velocity.pkl")
        ...
        # 7. trained model
        with open(self.output_dir / 'velocity_model.pkl', 'wb') as f:
            pickle.dump(self.model, f)
        print(f"  Generated: velocity_model.pkl")
    ...
    def _generate_config_txt(self):
        ...
        config_text += "TYPE: Head Velocity Speed Classifier\n"
        config_text += "Variables: HeadVelocityX, HeadVelocityY\n"
    ...
    def _generate_accuracy_txt(self):
        ...
        accuracy_text = "VELOCITY CLASSIFIER - RESULTS\n"
        accuracy_text += "=" * 60 + "\n\n"
        ...
        accuracy_text += "SPEED CATEGORIES:\n\n"
        ...
        for cat_name, stats in self.training_metadata['command_statistics'].items():
            ...
            accuracy_text += f"  Magnitude mean: {stats['magnitude_mean']:.2f}\n"
            accuracy_text += f"  Magnitude std: {stats['magnitude_std']:.2f}\n\n"
...
class InferencePipeline:
    """
    Inference pipeline for trained velocity model
    
    Loads trained model and PKL files for real-time classification.
    """
    
    def __init__(self, model_dir: str = "trained_velocity_models"):
    ...
    def _load_model_files(self):
        ...
        print("\n" + "="*80)
        print("LOADING TRAINED VELOCITY MODEL")
        print("="*80 + "\n")
        
        # Load model
        model_path = self.model_dir / 'velocity_model.pkl'
        if model_path.exists():
            with open(model_path, 'rb') as f:
                self.model = pickle.load(f)
            print(f"Loaded: velocity_model.pkl")
        else:
            raise FileNotFoundError(f"Model not found: {model_path}")
        
        # Load command mapping
        mapping_path = self.model_dir / 'command_mapping_velocity.pkl'
        if mapping_path.exists():
            with open(mapping_path, 'rb') as f:
                self.command_mapping = pickle.load(f)
            print(f"Loaded: command_mapping_velocity.pkl")
        
        # Load schema
        schema_path = self.model_dir / 'velocity_schema.pkl'
        if schema_path.exists():
            with open(schema_path, 'rb') as f:
                self.schema = pickle.load(f)
            print(f"Loaded: velocity_schema.pkl")
        
        # Load metadata
        metadata_path = self.model_dir / 'training_metadata_velocity.pkl'
        if metadata_path.exists():
            with open(metadata_path, 'rb') as f:
                self.metadata = pickle.load(f)
            print(f"Loaded: training_metadata_velocity.pkl")
        
        print("\n" + "="*80)
        print("MODEL LOADED SUCCESSFULLY")
        print("="*80 + "\n")
    
    def predict(self, velocity_x: np.ndarray, velocity_y: np.ndarray) -> Tuple[str, float]:
        """
        Predict speed category from head velocities
        
        Parameters
        ----------
        velocity_x, velocity_y : np.ndarray
            Head velocity sequences
        ...
        """
        sequence = (velocity_x, velocity_y)
    ...
    def predict_batch(self, sequences: List[Tuple[np.ndarray, np.ndarray]]) -> List[Tuple[str, float]]:
        ...
        sequences : list of tuple
            List of (velocity_x, velocity_y) sequences
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python velocity_classifier.py train [options]")
        print("  python velocity_classifier.py test")
        return
    ...
    if mode == 'train':
        # Training mode
        pipeline = TrainingPipeline(output_dir="trained_velocity_models")
        pipeline.run(
            n_samples_per_category=24,
            n_trees=100,
            max_depth=15,
            test_split=0.2
        )
        ...
        # Testing mode
        inference = InferencePipeline(model_dir="trained_velocity_models")
        
        # Generate test sample
        generator = SyntheticVelocityGenerator()
        test_sequence = generator.generate_category_dataset('MEDIA', 1)[0]
