#!/usr/bin/env python3
"""
Head Deviation Navigation Classification System
Proximity Forest 2.0 with MSM, SSDTW, and ERP Distance Metrics

This system trains and classifies head deviation patterns for wheelchair navigation
using 5 behavioral categories. Implements Proximity Forest 2.0 with enhanced
...
class MSM_Enhanced:
    ...
    def compute_distance(self, seq1: Tuple[np.ndarray, np.ndarray], 
        ...
        seq1, seq2 : tuple of np.ndarray
            Bivariate sequences (deviation_x, deviation_y)
...
@dataclass
class DeviationParameters:
    """Parameters for synthetic deviation generation"""
    mean: float
    std: float


class SyntheticDeviationGenerator:
    """
    Generates synthetic head deviation sequences for navigation classification
    
    Produces training data for 5 navigation categories based on head deviation
    patterns during wheelchair navigation.
    """
    
    DEVIATION_CONFIGS = {
        'NAVEGACION_EFICIENTE': DeviationParameters(mean=0.05, std=0.015),
        'NAVEGACION_DIRIGIDA': DeviationParameters(mean=0.18, std=0.05),
        'EXPLORACION_PAUSADA': DeviationParameters(mean=0.55, std=0.15),
        'EXPLORACION_ACTIVA': DeviationParameters(mean=1.5, std=0.35),
        'BUSQUEDA_REORIENTACION': DeviationParameters(mean=1.8, std=0.60)
    }
    ...
    def generate_category_dataset(self, category: str, n_samples: int) -> List[Tuple[np.ndarray, np.ndarray]]:
        ...
        ----------
        category : str
            Category name (must be in DEVIATION_CONFIGS)
        n_samples : int
        ...
        -------
        list of tuple
            List of (HeadDeviationX, HeadDeviationY) sequences
        """
        if category not in self.DEVIATION_CONFIGS:
            raise ValueError(f"Unknown category: {category}")
        
        config = self.DEVIATION_CONFIGS[category]
        sequences = []
        
        for _ in range(n_samples):
            # Generate base deviations
            dev_x = np.random.normal(config.mean, config.std, self.sequence_length)
            dev_y = np.random.normal(config.mean * 0.85, config.std * 0.95, self.sequence_length)
            
            # Random sign flip
            dev_x *= np.random.choice([-1, 1])
            dev_y *= np.random.choice([-1, 1])
            
            # Add small noise
            dev_x += np.random.uniform(-0.01, 0.01, self.sequence_length)
            dev_y += np.random.uniform(-0.01, 0.01, self.sequence_length)
            
            sequences.append((dev_x, dev_y))
    ...
    def generate_full_dataset(self, n_samples_per_category: int) -> Dict[str, List[Tuple[np.ndarray, np.ndarray]]]:
        ...
        for category in self.DEVIATION_CONFIGS.keys():
...
class ProximityForest:
    ...
    def __init__(self, n_trees: int = 100, max_depth: int = 12,
    ...
    def fit(self, X: List[Tuple[np.ndarray, np.ndarray]], y: np.ndarray):
        ...
        ----------
        X : list of tuple
            Training sequences (DeviationX, DeviationY)
        y : np.ndarray
            Training labels
        """
...
class TrainingPipeline:
    ...
    def __init__(self, output_dir: str = "trained_deviation_models"):
        self.output_dir = Path(output_dir)
        self.output_dir.mkdir(exist_ok=True)
        
        self.generator = SyntheticDeviationGenerator()
        self.model = None
    ...
    def run(self, n_samples_per_category: int = 24, n_trees: int = 100, 
        ...
        """
        print("\n" + "="*80)
        print("HEAD DEVIATION NAVIGATION CLASSIFIER - TRAINING PIPELINE")
        print("="*80 + "\n")
        
        # Generate data
        print("Generating synthetic deviation data...")
    ...
    def _collect_metadata(self, dataset: Dict, accuracy: float, training_time: float,
        ...
        for cat_name, sequences in dataset.items():
            ...
            command_stats[cat_name] = {
                'count': len(sequences),
                'deviation_x_mean': float(np.mean(x_values)),
                'deviation_y_mean': float(np.mean(y_values)),
                'deviation_x_std': float(np.std(x_values)),
                'deviation_y_std': float(np.std(y_values))
            }
    ...
    def _generate_pkl_files(self, dataset: Dict):
        ...
        # 1. pf2_deviation__2_.pkl (references in original format)
        deviation_references = {}
        for cat_name, sequences in dataset.items():
            # Store as list of combined magnitude arrays (to match original format)
            refs = []
            ...
            deviation_references[cat_name] = refs
        
        with open(self.output_dir / 'pf2_deviation__2_.pkl', 'wb') as f:
            pickle.dump(deviation_references, f)
        print(f"  Generated: pf2_deviation__2_.pkl")
        
        # 2. command_mapping_deviation.pkl
        with open(self.output_dir / 'command_mapping_deviation.pkl', 'wb') as f:
            pickle.dump(self.command_mapping, f)
        print(f"  Generated: command_mapping_deviation.pkl")
        
        # 3. deviation_schema.pkl
        schema = {
            'features': ['HeadDeviationX', 'HeadDeviationY'],
            'sequence_length': self.generator.sequence_length,
            'expected_dtypes': ['float64', 'float64']
        }
        
        with open(self.output_dir / 'deviation_schema.pkl', 'wb') as f:
            pickle.dump(schema, f)
        print(f"  Generated: deviation_schema.pkl")
        
        # 4. training_metadata_deviation.pkl
        with open(self.output_dir / 'training_metadata_deviation.pkl', 'wb') as f:
            pickle.dump(self.training_metadata, f)
        print(f"  Generated: training_metadata_deviation.pkl")
        ...
        # 7. trained model
        with open(self.output_dir / 'deviation_model.pkl', 'wb') as f:
            pickle.dump(self.model, f)
        print(f"  Generated: deviation_model.pkl")
    ...
    def _generate_config_txt(self):
        ...
        config_text += "TYPE: Head Deviation Navigation Classifier\n"
        config_text += "Variables: HeadDeviationX, HeadDeviationY\n"
    ...
    def _generate_accuracy_txt(self):
        ...
        accuracy_text = "DEVIATION CLASSIFIER - RESULTS\n"
        accuracy_text += "=" * 60 + "\n\n"
        ...
        for cat_name, stats in self.training_metadata['command_statistics'].items():
            ...
            accuracy_text += f"  DevX mean: {stats['deviation_x_mean']:.3f}\n"
            accuracy_text += f"  DevY mean: {stats['deviation_y_mean']:.3f}\n\n"
...
class InferencePipeline:
    """
    Inference pipeline for trained deviation model
    
    Loads trained model and PKL files for real-time classification.
    """
    
    def __init__(self, model_dir: str = "trained_deviation_models"):
    ...
    def _load_model_files(self):
        ...
        print("\n" + "="*80)
        print("LOADING TRAINED DEVIATION MODEL")
        print("="*80 + "\n")
        
        # Load model
        model_path = self.model_dir / 'deviation_model.pkl'
        if model_path.exists():
            with open(model_path, 'rb') as f:
                self.model = pickle.load(f)
            print(f"Loaded: deviation_model.pkl")
        else:
            raise FileNotFoundError(f"Model not found: {model_path}")
        
        # Load command mapping
        mapping_path = self.model_dir / 'command_mapping_deviation.pkl'
        if mapping_path.exists():
            with open(mapping_path, 'rb') as f:
                self.command_mapping = pickle.load(f)
            print(f"Loaded: command_mapping_deviation.pkl")
        
        # Load schema
        schema_path = self.model_dir / 'deviation_schema.pkl'
        if schema_path.exists():
            with open(schema_path, 'rb') as f:
                self.schema = pickle.load(f)
            print(f"Loaded: deviation_schema.pkl")
        
        # Load metadata
        metadata_path = self.model_dir / 'training_metadata_deviation.pkl'
        if metadata_path.exists():
            with open(metadata_path, 'rb') as f:
                self.metadata = pickle.load(f)
            print(f"Loaded: training_metadata_deviation.pkl")
        
        print("\n" + "="*80)
        print("MODEL LOADED SUCCESSFULLY")
        print("="*80 + "\n")
    
    def predict(self, deviation_x: np.ndarray, deviation_y: np.ndarray) -> Tuple[str, float]:
        """
        Predict navigation category from head deviations
        
        Parameters
        ----------
        deviation_x, deviation_y : np.ndarray
            Head deviation sequences
        ...
        """
        sequence = (deviation_x, deviation_y)
    ...
    def predict_batch(self, sequences: List[Tuple[np.ndarray, np.ndarray]]) -> List[Tuple[str, float]]:
        ...
        sequences : list of tuple
            List of (deviation_x, deviation_y) sequences
...
def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python deviation_classifier.py train [options]")
        print("  python deviation_classifier.py test")
        return
    ...
    if mode == 'train':
        # Training mode
        pipeline = TrainingPipeline(output_dir="trained_deviation_models")
        pipeline.run(
        ...
        # Testing mode
        inference = InferencePipeline(model_dir="trained_deviation_models")
        
        # Generate test sample
        generator = SyntheticDeviationGenerator()
