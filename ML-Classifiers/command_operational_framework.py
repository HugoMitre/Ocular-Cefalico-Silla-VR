#!/usr/bin/env python3
"""
Head Angle Command Classification System
Operational Framework for Real-Time and Batch Processing

This system provides comprehensive command classification for wheelchair navigation
through head angle pattern recognition. Built on Proximity Forest 2.0 with MSM,
SSDTW, and ERP distance metrics, it supports real-time TCP communication with Unity,
batch CSV processing, and synthetic data validation.
...
- Multi-distance metric fusion (MSM, SSDTW, ERP)
...
try:
    from wheelchair_command_classifier import InferencePipeline, SyntheticCommandGenerator
    CLASSIFIER_AVAILABLE = True
except ImportError:
    CLASSIFIER_AVAILABLE = False
    print("Warning: Command classifier module not available")
...
COMMAND_ANGLES = {
    'front': {'x': 0.0, 'y': 15.0},
    'back': {'x': 0.0, 'y': -85.0},
    'left_turn': {'x': -67.5, 'y': 0.0},
    'right_turn': {'x': 67.5, 'y': 0.0},
    'front_left_diagonal': {'x': -37.5, 'y': 15.0},
    'front_right_diagonal': {'x': 37.5, 'y': 15.0},
    'back_left_diagonal': {'x': -67.5, 'y': -85.0},
    'back_right_diagonal': {'x': 67.5, 'y': -85.0}
}

CONFIG_FILE = Path("command_config.json")
...
class SyntheticDataServer:
    ...
    Accepts connections for testing with generated synthetic command sequences,
    ...
    def _process_and_respond(self, client_socket: socket.socket, json_str: str, 
        ...
        try:
            ...
            print(f"AngleX range: [{np.min(angle_x):.2f}, {np.max(angle_x):.2f}]")
            print(f"AngleY range: [{np.min(angle_y):.2f}, {np.max(angle_y):.2f}]")
            print(f"\nClassifying...")
...
class RealDataServer:
    ...
    real-time wheelchair command classification.
    """
    ...
    def __init__(self, system, host: str = '0.0.0.0', port: int = 5557):
        ...
        self.category_distribution = defaultdict(lambda: {'total': 0, 'correct': 0, 'incorrect': 0})
    ...
    def _process_and_respond(self, client_socket: socket.socket, json_str: str, 
        ...
        try:
            ...
            with self.stats_lock:
                ...
                self.category_distribution[ground_truth]['total'] += 1
                if is_correct:
                    self.category_distribution[ground_truth]['correct'] += 1
                else:
                    self.category_distribution[ground_truth]['incorrect'] += 1
            
            status = "CORRECT" if is_correct else "INCORRECT"
            accuracy = (self.total_correct / self.total_classified * 100) if self.total_classified > 0 else 0
            ...
            print(f"  Time: {elapsed_ms:.1f}ms")
            print(f"  Status: {status}")
            print(f"  Overall accuracy: {accuracy:.1f}% ({self.total_correct}/{self.total_classified})")
            print(f"{'='*80}\n")
    ...
    def stop(self):
        ...
        print(f"Total received: {self.total_received}")
        print(f"Total classified: {self.total_classified}")
        print(f"Correct: {self.total_correct} ({self.total_correct/max(self.total_classified,1)*100:.1f}%)")
        print(f"Incorrect: {self.total_incorrect}")
        print(f"Errors: {self.total_errors}")
        print(f"Average time: {self.avg_time_ms:.1f}ms")
        
        if self.category_distribution:
            print(f"\nCOMMAND ACCURACY:")
            for command, stats in sorted(self.category_distribution.items()):
                total = stats['total']
                correct = stats['correct']
                accuracy = (correct / total * 100) if total > 0 else 0
                print(f"  {command:25} {correct:3}/{total:3} ({accuracy:5.1f}%)")
...
class CommandClassificationSystem:
    """
    ...
    def __init__(self, model_manager: ModelManager, config: SystemConfiguration):
        ...
        command_model = model_manager.get('proximity_forest_model')
    ...
    def _display_system_info(self):
        ...
        print(f"\n{'='*80}")
        print(f"COMMAND CLASSIFICATION SYSTEM")
        print(f"{'='*80}\n")
    ...
    def _display_batch_report(self, results: List[Dict], total_time: float, total_files: int):
        ...
        for command, count in distribution.most_common():
            percentage = (count / len(results)) * 100
            print(f"  {command:25} {count:4} ({percentage:5.1f}%)")
        
        print(f"\n{'='*80}\n")
    
    def validate_synthetic_data(self, command: str, n_sequences: int):
        ...
        generator = SyntheticCommandGenerator(sequence_length_range=(45, 59))
        results = []
        
        for i in range(1, n_sequences + 1):
            try:
                sequences = generator.generate_category_dataset(command, 1)
                if not sequences:
                    continue
                ...
                is_correct = prediction == command
                results.append(is_correct)
                
                status = "CORRECT" if is_correct else "INCORRECT"
                print(f"[{i:3d}/{n_sequences}] Ground truth: {command:25} | Prediction: {prediction:25} | {status}")
...
def main():
    ...
    print("\n" + "="*80)
    print(" HEAD ANGLE COMMAND CLASSIFICATION SYSTEM ".center(80))
    print("="*80)
    ...
    if not model_manager.get('proximity_forest_model'):
        print("\nNo valid models found. Exiting...")
        return
    
    config = SystemConfiguration()
    system = CommandClassificationSystem(model_manager, config)
    ...
    while True:
        ...
        if choice == '1':
            ...
            print("\nCommand Categories:")
            for i, command in enumerate(COMMAND_CATEGORIES, 1):
                angles = COMMAND_ANGLES[command]
                print(f"  {i}. {command:25} [X:{angles['x']:6.1f}, Y:{angles['y']:6.1f}]")
            
            try:
                idx = int(input("\nSelect command (1-8): ")) - 1
                if 0 <= idx < len(COMMAND_CATEGORIES):
                    command = COMMAND_CATEGORIES[idx]
                    n_sequences = int(input("Number of sequences: "))
                    if n_sequences > 0:
                        system.validate_synthetic_data(command, n_sequences)
            except:
                print("Invalid input")
